...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    $upload_directory = './image/seconds/';
    ...
    if (move_uploaded_file($image_front_tmp_name, $image_front_path)) {
        ...
        $query = "INSERT INTO seconds (name, price, image_url, watch_id, created_by, is_available) VALUES (?, ?, ?, ?, 1, 20)";
        if ($stmt = $conn->prepare($query)) {
            $stmt->bind_param("sdsi", $name, $price, $image_front_path, $watch_id);
            if ($stmt->execute()) {
                // Record inserted successfully
                $res["success"] = true;
                $res["msg"] = "Seconds added successfully.";
                // Fetch the inserted record to return it
                $inserted_id = $stmt->insert_id;
                $query = "SELECT * FROM seconds WHERE sid = ?";
                if ($stmt = $conn->prepare($query)) {
